"""Email provider for JARVIS Phase 26."""
...
logger = logging.getLogger("jarvis.communications.email")
...
class EmailProvider(CommunicationProvider):
    ...
    async def send_message(self, conversation_id: str, text: str, attachments: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        return {"success": True, "message_id": "mock-email-1"}
    ...
    async def start_call(self, contact_identifier: str) -> dict[str, Any]:
        return {"success": False, "error": "Email provider does not support calls"}
